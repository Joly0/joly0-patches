...
final class PlaylistRowTracker {
     ...
     */
    static boolean looksLikeEditablePlaylist(RecyclerView recyclerView) {
     ...
     */
    List<TrackedRow> update(RecyclerView recyclerView, List<PlaylistItem> items) {
    ...
    private static View findRow(View child) {
        try {
            ...
            // The helper throws rather than returning null when the name is not present.
            return null;
        }
    }
    ...
    private static void sortByTop(List<View> rows, List<String> titles) {
        for (int i = 1; i < rows.size(); i++) {
            View row = rows.get(i);
            String title = titles.get(i);
            final float top = rowTop(row);
            int j = i - 1;
            while (j >= 0 && rowTop(rows.get(j)) > top) {
    ...
    /**
     * Row top in RecyclerView coordinates, following any in-progress gesture.
     * <p>
     ...
     */
    static float rowTop(View row) {
        return accumulate(row, true);
    }

    /** Row left in RecyclerView coordinates, following any in-progress gesture. */
    static float rowLeft(View row) {
        return accumulate(row, false);
    }

    private static float accumulate(View row, boolean vertical) {
        float total = 0;
        View current = row;
        while (current != null) {
            total += vertical
    ...
    static void clearGutters(RecyclerView recyclerView) {
        for (int i = 0, count = recyclerView.getChildCount(); i < count; i++) {
            View row = findRow(recyclerView.getChildAt(i));
            if (row != null) {
                setGutter(row, 0);
     ...
     */
    record TrackedRow(View view, @Nullable PlaylistItem item) {
    }
}
