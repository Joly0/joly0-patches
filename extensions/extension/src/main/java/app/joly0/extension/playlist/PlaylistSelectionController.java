...
import app.joly0.extension.AuthUtils;
import app.joly0.extension.Logger;
import app.joly0.extension.PlaylistHeader;
...
import app.joly0.extension.requests.RemovePlaylistVideosRequest;

import static app.joly0.extension.Strings.str;

/**
 * Drives bulk selection for one playlist list: puts the overlay and the action bar in place,
 * loads the playlist contents, and performs the removal.
 * <p>
 * One instance per list handed to the hook. Several Litho lists are alive at once and most of
 * them are not playlists, so an instance stays dormant until the page header confirms playlist
 * rows are rendering.
 */
public final class PlaylistSelectionController {
    ...
    /** Null until a playlist has been fetched. */
    private String fetchedPlaylistId;

    public PlaylistSelectionController(ViewGroup recyclerView) {
    ...
    private void install() {
        Activity activity = Utils.activityOf(recyclerView);
        if (activity == null) {
            return;
        }
     ...
     * the Home button.
     *
     * @return the navigation bar, or null when it is not present.
     */
    ...
    private void maybeFetch() {
        final String playlistId = PlaylistHeader.getCurrentPlaylistId();
        ...
        if (AuthUtils.isNotLoggedIn()) {
            ...
            Utils.showToastShort(str("playlist_bulk_remove_not_logged_in"));
            return;
        }
    ...
    private void startFetch(String playlistId) {
        ...
        final Map<String, String> requestHeader = AuthUtils.getRequestHeaders();
        Utils.runOnBackgroundThread(() -> {
            ...
            Utils.runOnMainThread(() -> {
                ...
                if (items.isEmpty()) {
                    Utils.showToastShort(str("playlist_bulk_remove_load_failed"));
                }
                onSelectionChanged();
                if (overlay != null) {
                    overlay.invalidate();
    ...
    private void onRemove() {
        ...
        AlertDialog dialog = new AlertDialog.Builder(activity)
                .setTitle(selected.size() == 1
                        ? str("playlist_bulk_remove_confirm_title_one")
                        : str("playlist_bulk_remove_confirm_title", selected.size()))
                .setMessage(str("playlist_bulk_remove_confirm_message",
                        describeSelection(selected)))
                .setPositiveButton(str("playlist_bulk_remove_confirm_button"),
    ...
    private String describeSelection(List<PlaylistItem> selected) {
        ...
        if (selected.size() > shown) {
            builder.append(str("playlist_bulk_remove_confirm_more", selected.size() - shown));
        }
        return builder.toString();
    }

    private void performRemoval(String playlistId, List<PlaylistItem> selected) {
        Utils.showToastShort(str("playlist_bulk_remove_removing"));
